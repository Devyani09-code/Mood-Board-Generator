...
async function createMoodboard(boardType: "moodboard" | "brandboard", prompt: string, logoImageDataUrl?: string): Promise<unknown> {
  ...
  if (Array.isArray(parsed.layout)) {
    if (boardType === "brandboard") {
      ...
      const requiredLabels = ["Logo direction", "Sticker mark", "Logo alt", "Icon mark", "Mockup", "Pattern", "Fonts", "Mockup", "Mockup"];

      parsed.layout = parsed.layout.map((tile, index) => ({
        ...tile,
        type: index === 6 ? "text" : "image",
        label: requiredLabels[index] ?? tile.label,
      }));
    }
...
async function fetchStockImage(query: string, completeBrief: string): Promise<string | null> {
  const pexelsCandidates = await fetchPexelsImages(query);

  const candidates = pexelsCandidates.length > 0 ? pexelsCandidates : await fetchUnsplashImages(query);
...
async function fetchUnsplashImages(query: string): Promise<ImageCandidate[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[unsplash] request failed with ${response.status} ${response.statusText}`);
      return [];
    }
...
async function fetchPexelsImages(query: string): Promise<ImageCandidate[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[pexels] request failed with ${response.status} ${response.statusText}`);
      return [];
    }
...
This change must be clearly visible in the result: update the specific tiles it affects (their label, value, and/or accent color), and reflect it in the direction paragraph and keywords too.

Do not return a board that is nearly identical to the input — a refinement with no noticeable difference is a failure.
