...
async function createMoodboard(boardType: "moodboard" | "brandboard", prompt: string, logoImageDataUrl?: string): Promise<unknown> {
  ...
  const parsed = JSON.parse(content) as {
    ...
    keywords?: string[];
    direction?: string;
    layout?: Array<{ type: string; label: string; value: string; accent?: string | null; size?: string; imageUrl?: string | null }>;
  };
...
async function fetchUnsplashImages(query: string): Promise<ImageCandidate[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[unsplash] API returned ${response.status}: ${response.statusText}`);
      return [];
    }
...
async function fetchPexelsImages(query: string): Promise<ImageCandidate[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[pexels] API returned ${response.status}: ${response.statusText}`);
      return [];
    }
